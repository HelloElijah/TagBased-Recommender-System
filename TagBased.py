...
class MF():
    ...
    def __init__(self, train_data, test_data, user_tag, recipe_tag, K, alpha, lumbda, iterations):
        """
        ...
        - lumbda (float)  : regularization parameter
        """
    ...
    def train(self):
        ...
        for i in range(self.iterations):
            ...
            self.sgd()
            mse, ab_error = self.mse()
            training_process.append((i, mse))

            print("Iteration: %d ; error = %.4f" % (i+1, mse))
            print("Absolute Error is {:.3f}\n".format(ab_error))
    ...
    def mse(self):
        ...
        return np.sqrt(error/ len(xs)), ab_error/len(xs)
    ...
    def sgd(self):
        ...
        for i, j, r in self.samples:
            ...
            # Update biases
            self.b_u[i] += self.alpha * (e - self.lumbda * self.b_u[i])
            self.b_i[j] += self.alpha * (e - self.lumbda * self.b_i[j])

            # Create copy of row of P since we need to update it but use older values for update on Q
            P_i = self.P[i, :][:]
            Q_i = self.Q[j, :][:]

            # Fetch all the tags from User i, to calculate sum of vector
            tag_flag = False
            num_user_tags = 1
            ...
            if i in self.user_tag[0]:
                ...
                for tag_count in range(num_user_tags):
                    ...
                    # Small regularization for Weight
                    for k in range(self.K):
                        if (self.Utag[abs(tag_id)-1][k] * self.P[i][k] < 0 and tag_id > 0) or (self.Utag[abs(tag_id)-1][k] * self.P[i][k] > 0 and tag_id < 0):
                            self.Utag[abs(tag_id)-1][k] = self.Utag[abs(tag_id)-1][k] * -1
                    sum_user_tag += self.Utag[abs(tag_id)-1, :]
            
            # Fetch all the tags attached to Recipe j, to calculate sum of vector
            ...
            self.P[i, :] += self.alpha * (e * ( Q_i + sum_recipe_tag/num_recipe_tags ) - self.lumbda * P_i)
            self.Q[j, :] += self.alpha * (e * ( P_i + sum_user_tag/num_user_tags )-  self.lumbda * Q_i)

            # Fetch all the tags attached to User i, to update the user tag matrix
            if tag_flag == True:
                for tag_count in range(num_user_tags):
                    tag_id = self.user_tag[tag_row+1][tag_count]
                    self.Utag[abs(tag_id)-1, :] += self.alpha * ( e / num_user_tags * ( Q_i + sum_recipe_tag/num_recipe_tags ) - self.lumbda * self.Utag[abs(tag_id)-1, :]  )

            # Fetch all the tags attached to Recipe j, to update the recipe tag matrix
            for tag_count in range(num_recipe_tags):
                tag_id = self.recipe_tag[j][tag_count]     
                # Need to -1 to get actual row number of tag
                self.Rtag[tag_id-1, :] += self.alpha * ( e / num_recipe_tags * ( P_i + sum_user_tag/num_user_tags ) - self.lumbda * self.Rtag[tag_id-1, :]  )
...
def print_tag_matrix(matrix):
    for i in range(1, 4):
        # (1, 4) means User 2 to User 4
        print("User: {}: ".format(matrix[0][i-1]))
        for j in range(len(matrix[i])):
...
def load_user_tag(data):
    ...
    ID will be negative if it is a negative tag(aka: user does not like this tag)
    """
    ...
    for i in range(1, 50):
        ...
        if user_id not in matrix[0]:
            ...
            matrix[len(matrix[0])].append(tag_id*weight)
        else:
            index = matrix[0].index(user_id)
            matrix[index+1].append(tag_id*weight)

    return matrix

# Split data into training and testing set and use a matrix form
...
mf = MF(train, test, user_tag_matrix, recipe_tag_matrix, K=9, alpha=0.02, lumbda=0.005, iterations=7)
