...
class MF():
    ...
    def mse(self):
        ...
        # print("Absolute Error is {:.3f}".format(ab_error/len(xs)))
    ...
    def sgd(self):
        ...
        for i, j, r in self.samples:
            ...
            # Fetch all the tags from to User i, to calculate sum of vector
            ...
            if tag_flag == True:
                for tag_count in range(num_user_tags):
                    tag_id = self.user_tag[tag_row+1][tag_count]
                    self.Utag[abs(tag_id)-1, :] += self.alpha * ( e / num_user_tags * ( Q_i + sum_recipe_tag/num_recipe_tags ) - self.beta * self.Utag[abs(tag_id)-1, :]  )
...
# k_list = [6,7,8]
# a_list = [0.027,0.028,0.029]
# b_list = [0.004,0.005,0.006]
# list = []
...
#                 mf = MF(train, test, user_tag_matrix, recipe_tag_matrix, K=k, alpha=a, beta=b, iterations=3)
#                 training_process = mf.train()
#                 mse = mf.mse()
#                 sum += mse

#             print()
#             print("mse: {:.4f} when K = {}, alpha = {}, beta = {}".format(sum/5, k, a, b))
#             list.append(sum/5)
...
for i in range(10):
    ...
    mf = MF(train, test, user_tag_matrix, recipe_tag_matrix, K=9, alpha=0.03, beta=0.005, iterations=10)
